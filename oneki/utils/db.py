...
class _DataBase(ABC):
    def __init__(self, **kwargs):
        ...
        self._document: DocumentReference = self._collection.document(document) if document is not None else None

        subcollection = kwargs.get("subcollection")
        self._subcollection: CollectionReference = self._document.collection(subcollection) if subcollection is not None else None
...
class Collection(_DataBase):
    ...
    def set(self, id, **kwargs): 
        if self._subcollection is not None: 
            self._subcollection.add(kwargs, str(id))
        else: 
            self._collection.add(kwargs, str(id))
...
class Document(_DataBase):
    def __init__(self, **kwargs):
        ...
        self._subdocument = self._subcollection.document(kwargs["subdocument"]) if subcollection is not None else None
